...
import { Search, Plus, X, Trash2, CheckCircle, AlertTriangle, XCircle, LogIn } from 'lucide-react';
...
const Projects = () => {
  ...
  const [alertModal, setAlertModal] = useState({
    show: false,
    type: 'success', // 'success', 'error', 'warning', 'confirm', 'login'
    title: '',
    message: '',
    onConfirm: null,
  });
  ...
  const handleAddProject = () => {
    ...
    if (!isAuthenticated) {
      showAlert('login', 'Login Required', 'Please login as admin to add new projects');
      return;
    }
  ...
  const handleEditProject = (project) => {
    if (!isAuthenticated) {
      showAlert('login', 'Login Required', 'Please login as admin to edit projects');
      return;
    }
  ...
  const handleDeleteProject = async (projectId) => {
    if (!isAuthenticated) {
      showAlert('login', 'Login Required', 'Please login as admin to delete projects');
      return;
    }
